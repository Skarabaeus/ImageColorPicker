...
this.$wrapper.append(this.$toolbox);this.$toolbox.append(this.$currentColor);this.$toolbox.append(this.$selectedColor);var a=null;if(typeof this.$canvas.get(0).getContext==="function")a=this.$canvas.get(0).getContext("2d");else if(typeof G_vmlCanvasManager!=="undefined"){G_vmlCanvasManager.initElement(this.$canvas.get(0));a=this.$canvas.get(0).getContext("2d")}else{this.destroy();throw new Error("ImageColor Picker: Can't get canvas context. Use Firefox, Chrome or include excanvas to your project.");
}var b=new Image;b.src=this.element.attr("src");this.$canvas.attr("width",b.width);this.$canvas.attr("height",b.height);a.drawImage(b,0,0);try{try{this.imageData=a.getImageData(0,0,b.width,b.height)}catch(g){netscape.security.PrivilegeManager.enablePrivilege("UniversalBrowserRead");this.imageData=a.getImageData(0,0,b.width,b.height)}}catch(e){this.destroy();throw new Error("ImageColor Picker: Unable to access image data. This could be either due to the browser you are using (IE doesn't work) or image and script are saved on different servers or you run the script locally. The exception is: "+
e);}this.element.hide();var f=this;this.$canvas.bind("mousemove",function(c){var d=f.$canvas.offset();c=((c.pageY-d.top)*b.width+(c.pageX-d.left))*4;d=f.imageData;f.$currentColor.css("backgroundColor","rgba("+d.data[c]+", "+d.data[c+1]+", "+d.data[c+2]+", "+d.data[c+3]+")")});this.$canvas.bind("click",function(c){var d=f.$canvas.offset();c=((c.pageY-d.top)*b.width+(c.pageX-d.left))*4;d=f.imageData;f.$selectedColor.css("backgroundColor","rgba("+d.data[c]+", "+d.data[c+1]+", "+d.data[c+2]+", "+d.data[c+
3]+")");f._trigger("afterColorSelected",0,f.selectedColor())});this.$canvas.bind("mouseleave",function(){f.$currentColor.css("backgroundColor","#fff")});$(window).unload(function(){f.destroy()})}});
